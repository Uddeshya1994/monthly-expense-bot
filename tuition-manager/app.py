        ...
        if st.form_submit_button("Add Student"):
            if not name or not cls:
                ...
                for board in ["CBSE", "ICSE"]:
                    class_folder = os.path.join(BASE_DIR, board, cls.strip())
                    os.makedirs(class_folder, exist_ok=True)
                    if board in SYLLABUS and cls.strip() in SYLLABUS[board]:
                        for subject in SYLLABUS[board][cls.strip()]:
                            subject_folder = os.path.join(class_folder, subject)
                            os.makedirs(subject_folder, exist_ok=True)
                st.success(f"Student added and folders created for Class {cls}")
                st.rerun()
